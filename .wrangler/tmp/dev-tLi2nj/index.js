...
// .wrangler/tmp/bundle-LYp8vt/strip-cf-connecting-ip-header.js
...
var SessionsStore = class extends DurableObject {
  ...
  async joinSession(pickupCode) {
    ...
    if (!session)
      return false;
    const connectionKey = `${pickupCode}-receiver`;
  ...
  async cleanupSession(pickupCode) {
    ...
    await this.saveSessions();
    activeConnections.delete(`${pickupCode}-sender`);
    activeConnections.delete(`${pickupCode}-receiver`);
  }
};
...
async function handleWebSocket(request, env) {
    ...
    try {
      ...
      switch (data.type) {
        case "create-session":
          ...
          const senderKey = `${newPickupCode}-sender`;
          activeConnections.set(senderKey, client);
        ...
        case "join-session":
          ...
          if (joinSession) {
            const receiverKey = `${pickupCode}-receiver`;
            if (!activeConnections.has(receiverKey)) {
              activeConnections.set(receiverKey, client);
              const senderKey2 = `${pickupCode}-sender`;
              const senderSocket = activeConnections.get(senderKey2);
              if (senderSocket) {
              ...
              client.send(JSON.stringify({
                type: "session-joined",
                data: { success: false, message: "\u8BE5\u53D6\u4EF6\u7801\u5DF2\u88AB\u4F7F\u7528" }
              }));
              client.close();
            }
          } else {
        ...
        case "file-info":
          ...
          if (fileSession) {
            ...
            const receiverKey = `${data.data.pickupCode}-receiver`;
            const receiverSocket = activeConnections.get(receiverKey);
            if (receiverSocket) {
        ...
        case "file-chunk":
          ...
          if (chunkSession) {
            ...
            if (data.data.isLast) {
              const senderKey2 = `${data.data.pickupCode}-sender`;
              const senderSocket = activeConnections.get(senderKey2);
              if (senderSocket) {
                senderSocket.send(JSON.stringify({
  ...
  client.onclose = async () => {
    if (pickupCode) {
      const connectionKey = `${pickupCode}-${clientType}`;
      activeConnections.delete(connectionKey);
      const otherType = clientType === "sender" ? "receiver" : "sender";
      const otherKey = `${pickupCode}-${otherType}`;
      const otherSocket = activeConnections.get(otherKey);
      if (otherSocket) {
        otherSocket.send(JSON.stringify({
          type: `${clientType}-disconnected`,
          data: { pickupCode }
        }));
      }
      if (!activeConnections.has(`${pickupCode}-sender`) && !activeConnections.has(`${pickupCode}-receiver`)) {
        await sessionsStore.cleanupSession(pickupCode);
      }
    }
  };
...
// .wrangler/tmp/bundle-LYp8vt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LYp8vt/middleware-loader.entry.ts
